...
const CheckoutForm : React.SFC<Props> = ({ form, theme }) => {
  ...
  return (
    <StyledAuthForm onSubmit={handleSubmit}>
      ...
      >
        <Form.Item>
        <Button type="primary" onClick={() => setDrawerOpen(true)}>Review Cart</Button>
        <Drawer
        title="Items In Cart"
        placement="left"
        closable={true}
        ...
        <Form.Item label="Billing Address">
          {getFieldDecorator("billing", {
            rules: [
              { required: false, message: "Please input a billing address!" }
            ]
          })(
            <Input
              prefix={<Icon type="home" style={{ color: "rgba(0,0,0,.25)" }} />}
        ...
        <Form.Item label="Shipping Method">
          {getFieldDecorator("radio-group", {
            ...
            <Radio.Group name="shipMethod" onChange={(e: {target: { value: number}}) => {
